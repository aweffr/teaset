...
export default class OverlayExample extends NavigationPage {
  ...
  showPopover(view, direction, align) {
    ...
    view.measure((x, y, width, height, pageX, pageY) => {
      ...
      let overlayView = (
        <Overlay.PopoverView popoverStyle={popoverStyle} fromBounds={fromBounds} direction={direction} align={align}
  ...
  renderPage() {
    ...
    return (
      <ScrollView style={{flex: 1}}>
        ...
        <ListRow title='Translucent modal' onPress={() => this.showDefault(false, true, 'Translucent modal')}
                 bottomSeparator='full'/>
        <View style={{height: 20}}/>
        <ListRow title='Pull from bottom' onPress={() => this.showPull('bottom', false, 'Pull from bottom')}
                 topSeparator='full'/>
        ...
        <ListRow title='Pull and translate'
                 onPress={() => this.showPull('left', false, 'Pull and translate', 'translate')}
                 bottomSeparator='full'/>
        <View style={{height: 20}}/>
          ...
          detail={
            <View>
              <View style={{paddingTop: 16, paddingBottom: 8, flexDirection: 'row', justifyContent: 'space-between'}}>
                <Checkbox title='Black' ref='black' checked={this.state.black}
                          onChange={value => this.setState({black: value})}/>
                <Checkbox title='Shadow' ref='shadow' checked={this.state.shadow}
                          onChange={value => this.setState({shadow: value})}/>
                <Checkbox title='Show arrow' ref='showArrow' checked={this.state.showArrow}
                          onChange={value => this.setState({showArrow: value})}/>
              </View>

              <View style={{paddingTop: 8, flexDirection: 'row', justifyContent: 'space-between'}}>
                <Button title='down start' ref='downstart'
                        onPress={() => this.showPopover(this.refs['downstart'], 'down', 'start')}/>
                <Button title='down center' ref='downcenter'
                        onPress={() => this.showPopover(this.refs['downcenter'], 'down', 'center')}/>
                <Button title='down end' ref='downend'
                        onPress={() => this.showPopover(this.refs['downend'], 'down', 'end')}/>
              </View>
              <View style={{paddingTop: 8, flexDirection: 'row', justifyContent: 'space-between'}}>
                <Button title='right start' ref='rightstart'
                        onPress={() => this.showPopover(this.refs['rightstart'], 'right', 'start')}/>
                <Button title='left start' ref='leftstart'
                        onPress={() => this.showPopover(this.refs['leftstart'], 'left', 'start')}/>
              </View>
              <View style={{paddingTop: 8, flexDirection: 'row', justifyContent: 'space-between'}}>
                <Button title='right center' ref='rightcenter'
                        onPress={() => this.showPopover(this.refs['rightcenter'], 'right', 'center')}/>
                <Button title='left center' ref='leftcenter'
                        onPress={() => this.showPopover(this.refs['leftcenter'], 'left', 'center')}/>
              </View>
              <View style={{paddingTop: 8, flexDirection: 'row', justifyContent: 'space-between'}}>
                <Button title='right end' ref='rightend'
                        onPress={() => this.showPopover(this.refs['rightend'], 'right', 'end')}/>
                <Button title='left end' ref='leftend'
                        onPress={() => this.showPopover(this.refs['leftend'], 'left', 'end')}/>
              </View>
              <View style={{paddingTop: 8, flexDirection: 'row', justifyContent: 'space-between'}}>
                <Button title='up start' ref='upstart'
                        onPress={() => this.showPopover(this.refs['upstart'], 'up', 'start')}/>
                <Button title='up center' ref='upcenter'
                        onPress={() => this.showPopover(this.refs['upcenter'], 'up', 'center')}/>
